CREATE OR REPLACE TABLE `derived.property_tile_info` AS (
    ...
    -- Latest model prediction per property (#12 may write multiple
    -- prediction rows over time; pick the most recent per property_id).
    ...
    LEFT JOIN current_predictions AS cp
        ON o.property_id = cp.property_id
    WHERE
